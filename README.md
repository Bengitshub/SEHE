...
| Reviews (rebuild, Sep 2026) | `SEHE-reviews-page_v2.txt` | single widget; live Trustpilot Micro Combo + 12 selected reviews verbatim + earlier guest feedback; built on `reviews-new` — see the runbook |
| Contact (rebuild, Sep 2026) | `SEHE-contact-page_v2.txt` | THREE widgets (Blocks A/B/C, split into `LATEST/contact-page-block*.txt`) around the NATIVE Duda form, which is never rebuilt or moved |
| About · FAQ · Gallery · Himalayan Trust (rebuild, Sep 2026) | `SEHE-about-page_v2.txt` · `SEHE-faq-page_v2.txt` · `SEHE-gallery-page_v1.txt` · `SEHE-himalayan-trust-page_v2.txt` | single widget each; copy ported verbatim from the live pages |
